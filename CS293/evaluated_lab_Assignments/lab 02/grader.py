...
exam                 = "Lab 02"
problem              = "reverse"
prob_files           = ['main.cpp','linked-list.h','linked-list.cpp']
student_files        = ['reverse.cpp']
allowed_include      = ['linked-list.h']
...
if act in ['compile','policy','run','results','grade','package','email','all']:
    ...
    if act in ['grade','all']:
        ...
        # print( full[ full["Compile Success"] == "PASS-WITH-INCLUDES" ].index )
        # exit()
        #----------------------------------
        # Compute total score 
        #----------------------------------
        ...
        full.loc[ full['Compile Success'] == "PASS-WITH-INCLUDES", 'total'] = 0
        full.loc[ full['Policy Error'   ] == True,                 'total'] = 0

        # full['total'] = np.where(  full['total'] >= 8,                       2,
        #                  np.where((full['total'] > 4) & (full['total'] < 8), 1,
        #                                                                      0)
        #                          )

        full['total'] = np.where(                       full['total'] >= 9,   4,
                        np.where(                       full['total'] == 8,   3,
                        np.where( (full['total'] > 5) & (full['total'] < 8),  1,
                                  0
                                  )))

        print(full[['Compile Success','Policy Error']+tests+['total']])
