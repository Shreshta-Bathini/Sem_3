...
exam                 = "Lab 03"
problem              = "crc"
prob_files           = ['main.cpp','crc.h']
student_files        = ['crc.cpp']
allowed_include      = ['crc.h']
...
if act in ['compile','policy','run','results','grade','package','email','all']:
    ...
    if act in ['grade','all']:
        ...
        full['total'] = np.where(    full['total'] >= 10,                       2,
                        np.where(   (full['total'] > 6) & (full['total'] < 10), 1,
                                                                                0))
        
        # full.loc[ full['Compile Success'] == "PASS-WITH-INCLUDES", 'total'] = 0
        # full.loc[ full['Policy Error'   ] == True,                 'total'] = 0

        # full['total'] = np.where(full['total'] >= 10, 2, 0)
